...
namespace
{
	options_description getInput( StabilityTestConfiguration::InputOptions & out )
	{
		// default sample
		int defSample = (3 * rand() / RAND_MAX) + 1;
	    options_description input( "Input" );	
	    input.add_options()
	    ( "sample", value<int>( & out.sample )->default_value( defSample ), "Test sample to process" )
	    ;

		return input; 
	}
	
	options_description getOutput( StabilityTestConfiguration::OutputOptions & out )
	{
...
StabilityTestConfiguration::StabilityTestConfiguration()
{
...
//void StabilityTestConfiguration::parseAllConfigFiles_() 
//{
//	WdbConfiguration::parseAllConfigFiles_();
//}

void StabilityTestConfiguration::parse_( int argc, char ** argv )
{
...
void StabilityTestConfiguration::postParseHook()
{
