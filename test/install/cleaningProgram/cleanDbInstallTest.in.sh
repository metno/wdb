...
list=`ls __WDB_BUILDDIR__/test/install/cleaningProgram/*.test`;
...
if test "$XML_OPT" != "-x"; then
	echo "1..$TOTAL_TESTS"
	echo "#"
	echo "# cleaningProgram Installation Tests"
	echo "#"
fi
...
if test "$XML_OPT" != "-x"; then    
	echo "#"
	echo "# cleaningProgram Installation Tests done"
	...
	# Create Header
	echo -e -n "<testsuite name=\"Cleaning Program Install Tests\" tests=\"$RUN_TESTS\" failures=\"$TOTALFAIL_TESTS\" errors=\"$ERROR_TESTS\" time=\"$totalTime\">"
	# Test Reports	
	echo -e $XML_OUT	
	# Closer
	echo -e "</testsuite>"
