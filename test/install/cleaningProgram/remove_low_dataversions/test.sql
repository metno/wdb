...
DECLARE
	row_count int;
BEGIN
	SELECT count(*) INTO row_count FROM wci.read(NULL,NULL, NULL,NULL, NULL,NULL, NULL,returntype);
	IF NOT row_count = expected THEN
		RAISE EXCEPTION 'Fond % rows, expected %: %', row_count, expected, message;
	END IF;
END;
$BODY$
LANGUAGE plpgsql;
...
SELECT clean_dataversions.clean();
...
SELECT assert_result_count(NULL::wci.returnfloat, 3, 'Wrong number of float rows left after clean - after a random row was manually removed');
ROLLBACK TO SAVEPOINT setup_complete;
