...
#!/bin/sh
# Setup environment
source __WDB_BUILDDIR__/test/install/loadingProgram/feltLoad/testEnvironment.sh
...
list=`ls __WDB_BUILDDIR__/test/install/loadingProgram/feltLoad/*.test`;
...
if test "$XML_OPT" != "-x"; then
	echo "1..$TOTAL_TESTS"
	echo "#"
	echo "# feltLoad Installation Tests"
	echo "#"
fi
...
if test "$XML_OPT" != "-x"; then    
	echo "#"
	echo "# feltLoad Installation Tests done"
	...
	# Create Header
	echo -e -n "<testsuite name=\"FeltLoad Install Tests\" tests=\"$RUN_TESTS\" failures=\"$TOTALFAIL_TESTS\" errors=\"$ERROR_TESTS\" time=\"$totalTime\">"
	# Test Reports	
	echo -e $XML_OUT	
	# Closer
	echo -e "</testsuite>"
