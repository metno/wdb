...
inline void assertInitialized(const WciSession & s) {
  if (!s.isInitialized())
    ereport(ERROR, (errcode(ERRCODE_RAISE_EXCEPTION), errmsg("wci has not been initialized.")));
}
...
Datum wciSession_setRole(PG_FUNCTION_ARGS) {
  ...
  if (!boost::regex_match(sRole, matchRole)) {
    ...
    ereport(ERROR, (errcode(ERRCODE_INVALID_ROLE_SPECIFICATION), errmsg("%s", sMsg.c_str())));
  }
  setRole(sRole.c_str());
