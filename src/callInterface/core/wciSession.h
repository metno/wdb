...
public:
	 ...
	 */
	static WciSession & get() { return session_; }
	...
	bool isInitialized() const { return not user().empty(); }
	
	/// Get the user name provided with the initialize call.
	const std::string & user() const { return user_; }
	
	/// Get wanted data provider codespace 
	int dataProviderNs() const { return dataProviderNs_; }
	
	/// Get wanted parameter codespace
    int placeNs() const { return placeNs_; }
    
    /// Get wanted level parameter codespace
	int parameterNs() const { return parameterNs_; }
