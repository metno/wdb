...
CREATE OR REPLACE FUNCTION __WCI_SCHEMA__.cache_file(id bigint)
RETURNS void AS
$BODY$
DECLARE
	fileExists bool := false;
BEGIN
	...
	IF NOT FOUND OR fileExists=false THEN
		RAISE EXCEPTION 'No such file: %', id;
	END IF;
	PERFORM __WDB_SCHEMA__.cache_file_impl(id);
END;
$BODY$
SECURITY DEFINER
LANGUAGE plpgsql VOLATILE;
...
CREATE OR REPLACE FUNCTION __WDB_SCHEMA__.cache_file_impl(id bigint)
RETURNS void AS
'__WDB_LIBDIR__/__WCI_LIB__', 'cache_file'
LANGUAGE C VOLATILE;
